...
//@formatter:on
    ...
    static {
        for (final Rotator rotator : Rotator.values()) {
    ...
    public List<BlockOffset> getRotated(BlockOffset offset) {
        final List<BlockOffset> rotoffsets = new ArrayList<BlockOffset>();

        for (final BlockOffset rotcount : rotlist) {
            rotoffsets.add(rotateXYZ(offset, rotcount.x, rotcount.y, rotcount.z));
        }
